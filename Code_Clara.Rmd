...
g + geom_bar(position="fill", width = 0.7) + labs(title="Histogram on Education in Italy",subtitle="Educational attainment in the different Geographical Areas", x= "Geographical Area", y= "Percentage", fill="Educational Attainment") + stat_count(geom = "text", aes(label = paste(round(after_stat(count)/sum(after_stat(count))*100), "%")), position=position_fill(vjust=0.5), colour="white") + coord_flip()
...
dfg %>%
  group_by(df.IregLabelled) %>%
  summarise(
    median_regincome = sort(rep(patient_age,number_patients))[length(rep(patient_age,number_patients))/2]
  )
```
...
reg_income <- aggregate(dfg$Income, by=list(dfg$IregLabelled), FUN=mean)  # aggregate
